<div class="comment-system">
            ...
            <form wire:submit.prevent="postComment" class="space-y-4">
                <div>
                    ...
                    <textarea 
                        ...
                        rows="4" 
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500 @error('newComment') border-red-500 @enderror"
                ...
                <div class="flex justify-end">
                    <button 
                        ...
                        class="px-6 py-2 bg-orange-600 text-white font-medium rounded-md hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
    ...
    @else
        <div class="mb-8 p-4 bg-gray-50 rounded-lg text-center">
            ...
            <a href="{{ route('login') }}" class="inline-flex items-center px-4 py-2 bg-orange-600 text-white font-medium rounded-md hover:bg-orange-700 transition-colors duration-200">
                Đăng nhập
            </a>
        </div>
    @endauth
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="bg-gray-50 rounded-lg p-4">
                                        ...
                                        @if(auth()->id() === $comment->user_id || auth()->user()->hasRole('admin'))
                                            <div class="flex items-center space-x-2">
                                                ...
                                                <button 
                                                    ...
                                                    wire:confirm="Bạn có chắc chắn muốn xóa bình luận này?"
                                ...
                                @if($editingComment === $comment->id)
                                    <div class="edit-form mt-2">
                                        <textarea 
                                            ...
                                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500 @error('editContent') border-red-500 @enderror" 
                                            rows="3"
                                        ></textarea>
                                        @error('editContent')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                ...
                                @if($showReplyForm === $comment->id)
                                    <div class="mt-4">
                                        <form wire:submit.prevent="postReply({{ $comment->id }})" class="space-y-3">
                                            <textarea 
                                                wire:model="replyContent"
                                                rows="2" 
                                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500 @error('replyContent') border-red-500 @enderror"
                                            ...
                                            <div class="flex space-x-2">
                                                <button 
                                                    ...
                                                    wire:loading.attr="disabled"
                                                    wire:loading.class="opacity-50 cursor-not-allowed"
                                                    class="px-4 py-2 bg-blue-600 text-white font-medium rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                                >
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div class="bg-white border border-gray-200 rounded-lg p-3">
                                                                ...
                                                                @if(auth()->id() === $reply->user_id || auth()->user()->hasRole('admin'))
                                                                    <div class="flex items-center space-x-2">
                                                                        ...
                                                                        <button 
                                                                            ...
                                                                            wire:confirm="Bạn có chắc chắn muốn xóa trả lời này?"
                                                                            class="text-gray-400 hover:text-red-600 transition-colors duration-200"
                                                                            title="Xóa"
                                                                        >
                                                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                        ...
                                                        <div class="flex items-center space-x-4 mt-2">
                                                            ...
                                                            <button 
                                                                wire:click="dislikeComment({{ $reply->id }})"
                                                                class="flex items-center space-x-1 text-gray-500 hover:text-red-600 transition-colors duration-200"
                                                            >
                                                                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
        ...
        @else
            <div class="text-center py-8 text-gray-500">
                <svg class="w-16 h-16 mx-auto mb-4 text-gray-300" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clip-rule="evenodd"></path>
                </svg>
                <p class="text-lg">Chưa có bình luận nào</p>
                <p class="text-sm">Hãy là người đầu tiên chia sẻ suy nghĩ của bạn!</p>
            </div>
        @endif
    </div>
</div>
